...
Project root (as submitted to GitHub / eDimension):
...
├─ Machine Learning Analysis of Flight Prices and Optimal Booking Time_v5.ipynb
├─ train.py                   # Training script: builds and saves the best model
├─ api.py                     # FastAPI app for serving predictions
├─ Clean_Dataset.csv          # Local data file (do NOT upload to eDimension)
├─ models/
...
> **Important:**
>
> * For **eDimension submission**, please **exclude** `Clean_Dataset.csv` and any very large model files if requested (only keep the code + report).
> * For **GitHub**, you may keep the CSV or just provide the Kaggle link; both are acceptable depending on the dataset license.
...
Example `requirements.txt` (you can adjust versions if needed):
...
```text
Saved best model pipeline to models/model.joblib
```
